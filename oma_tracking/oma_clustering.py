...
class ModeClusterer:
    ...
    columns: list = ['frequency', 'size', 'damping']
    ...
    def fit(
        ...
        """
        if not check_columns(self.columns, modal_data):
            raise ValueError('The modal data does not contain all the required columns.')
        # feature selection
        dbscan_data = modal_data[self.columns]
        ...
        dbscan_data = dbscan_data[self.columns]
    ...
    def predict(
        ...
        clustered_data = \
            self.dbscan_data[self.dbscan_data['labels'].isin(lbls)]\
                [self.dbscan_data[self.dbscan_data['labels'].isin(lbls)]['labels'] >= 0]
        # Reset the modes to start from 0
        codes = pd.factorize(clustered_data['labels'])
        clustered_data['labels'] = codes
        return clustered_data


def check_columns(columns: list, data: pd.DataFrame) -> bool:
    """
